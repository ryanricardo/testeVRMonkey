...
namespace HardShellStudios.InputManager
{
    ...
    public class hardManager : MonoBehaviour
    {
        ...
        public bool GetKey(string keyName)
        {
            ...
            if (!keyMaps.ContainsKey(keyName))
            {
                Debug.Log("Missing key " + keyName);
                return false;
            }
        ...
        public float GetAxis(string keyName, string keyName2, float gravity)
        {
            ...
            if (GetKey(keyName))
                keyMaps[keyName].keyValue += gravity * Time.unscaledDeltaTime;
            if (GetKey(keyName2))
                keyMaps[keyName].keyValue -= gravity * Time.unscaledDeltaTime;

            if (!GetKey(keyName) && !GetKey(keyName2))
                keyMaps[keyName].keyValue = Mathf.MoveTowards(keyMaps[keyName].keyValue, 0, gravity * Time.unscaledDeltaTime);
